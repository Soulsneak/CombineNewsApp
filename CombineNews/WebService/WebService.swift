...
class WebService{
    func getStoryById(storyId:Int) -> AnyPublisher<Story,Error>{
        guard let url = URL(string: "https://hacker-news.firebaseio.com/v0/item/\(storyId).json?print=pretty") else { fatalError("oof")}
    ...
    private func mergeStories(ids storyIds: [Int]) -> AnyPublisher<Story,Error>{
        ...
        let initalPublisher = getStoryById(storyId: storyIds[0])
